extends Node3D
class_name GoblinEngineerModel

## Goblin Engineer 模型包装器
...
func _ready():
	...
	# 🎨 应用工程师专属颜色（深绿色）
	set_model_color(Color(0.1, 0.5, 0.1))
...
func _remove_landscape():
	...
	if landscape:
		...
		print("GoblinEngineerModel: 已删除环境地面节点")
...
func apply_size_scale(size_value: float):
	...
	- size=18 → scale=0.18（工程师约1.8米高）
	- size=20 → scale=0.20（大型单位约2米高）
	"""
	var scale_factor = size_value * 0.01
	scale = Vector3(scale_factor, scale_factor, scale_factor)
	print("GoblinEngineerModel: 应用缩放 size=%f → scale=%f" % [size_value, scale_factor])
...
func set_model_color(color: Color):
	...
	@param color: 目标颜色（工程师默认使用深绿色区分苦工）
	"""
