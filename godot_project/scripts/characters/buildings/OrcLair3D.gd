...
func _setup_3d_config():
	...
	# 材质配置（原始风格）
	building_3d_config.wall_color = Color(0.4, 0.3, 0.2) # 棕色墙体
	building_3d_config.roof_color = Color(0.3, 0.2, 0.1) # 深棕色屋顶
	building_3d_config.floor_color = Color(0.5, 0.4, 0.3) # 浅棕色地板
...
func _get_building_template():
	"""获取兽人巢穴建筑模板"""
	var template = BuildingTemplateClass.new("兽人巢穴")
	template.building_type = BuildingTypes.BuildingType.ORC_LAIR
	template.description = "原始的3x3x3兽人巢穴，散发着野性的力量"
	
	# 创建原始结构
	template.create_orc_structure(BuildingTypes.BuildingType.ORC_LAIR)
