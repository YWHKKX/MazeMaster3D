...
# 样式配置
var style_config = {
	"name_color": Color(0.9, 0.9, 0.9, 0.8), # 稍微透明的白色
	"health_color": Color(0.2, 1.0, 0.2, 0.9), # 绿色
	"low_health_color": Color(1.0, 0.2, 0.2, 0.9), # 红色
	"faction_color": Color(1.0, 1.0, 0.2, 0.8), # 稍微透明的黄色
...
func _create_3d_labels() -> void:
	...
	if display_config.show_names:
		...
		name_label.pixel_size = 0.005 # 🔧 缩小名称字体大小
	...
	if display_config.show_health:
		...
		health_label.pixel_size = 0.004 # 🔧 缩小生命值字体大小
	...
	if display_config.show_faction:
		...
		faction_label.pixel_size = 0.004 # 🔧 缩小阵营字体大小
...
func _update_position() -> void:
	...
	if name_label:
		...
		offset += 0.2 # 🔧 缩小名称和生命值之间的间距
	
	if health_label:
		health_label.position = Vector3(0, offset, 0)
		offset += 0.15 # 🔧 缩小生命值和阵营之间的间距
...
func _update_content(lod_level: int) -> void:
	...
	if health_label and display_config.show_health and lod_level >= 1:
		...
		if health_percent > 0.6:
			health_label.modulate = Color(0.2, 1.0, 0.2, 0.9) # 绿色，稍微透明
		elif health_percent > 0.3:
			health_label.modulate = Color(1.0, 1.0, 0.2, 0.9) # 黄色，稍微透明
		else:
			health_label.modulate = Color(1.0, 0.2, 0.2, 0.9) # 红色，稍微透明
	else:
		if health_label:
			health_label.text = ""
...
## 为指定单位创建名称显示
static func create_for_unit(unit: CharacterBase, config: Dictionary = {}) -> UnitDisplay:
	"""为指定单位创建名称显示"""
...
## 批量创建单位名称显示
static func create_for_units(units: Array[CharacterBase], config: Dictionary = {}) -> Array[UnitDisplay]:
	"""批量创建单位名称显示"""
