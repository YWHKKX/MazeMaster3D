...
func generate_cave_resources(region: EcosystemRegion) -> Array[ResourceSpawn]:
	"""生成洞穴资源"""
	...
	for i in range(iron_count):
		...
		var resource = ResourceSpawn.new(ResourceTypes.ResourceType.IRON_ORE, Vector3(pos.x, 0, pos.y), amount, 900.0)
	...
	for i in range(gold_count):
		...
		var resource = ResourceSpawn.new(ResourceTypes.ResourceType.GOLD_ORE, Vector3(pos.x, 0, pos.y), amount, 1200.0)
		resources.append(resource)
	...
	for i in range(gem_count):
		...
		var resource = ResourceSpawn.new(ResourceTypes.ResourceType.GEM, Vector3(pos.x, 0, pos.y), amount, 1800.0)
		resources.append(resource)
	...
	for i in range(mushroom_count):
		...
		var resource = ResourceSpawn.new(ResourceTypes.ResourceType.MUSHROOM, Vector3(pos.x, 0, pos.y), amount, 360.0)
		resources.append(resource)
...
func generate_cave_creatures(region: EcosystemRegion) -> Array[CreatureSpawn]:
	"""生成洞穴生物"""
	...
	for i in range(rat_count):
		...
		var creature = CreatureSpawn.new(CreatureTypes.CreatureType.GIANT_RAT, Vector3(pos.x, 0, pos.y), level, true, 480.0)
...
func generate_cave_features(region: EcosystemRegion) -> Array[RegionFeature]:
	"""生成洞穴特殊特性"""
	...
	for i in range(stalactite_count):
		var pos = region.get_random_point()
		var feature = RegionFeature.new("stalactite", pos, "古老的钟乳石，可能有矿物")
	...
	for i in range(crystal_cave_count):
		var pos = region.get_random_point()
		var feature = RegionFeature.new("crystal_cave", pos, "闪闪发光的水晶洞，宝石丰富")
		features.append(feature)
	...
	for i in range(deep_cave_count):
		var pos = region.get_random_point()
		var feature = RegionFeature.new("deep_cave", pos, "深不见底的洞穴，可能有稀有矿物")
		features.append(feature)
	...
	for i in range(underground_lake_count):
		var pos = region.get_random_point()
		var feature = RegionFeature.new("underground_lake", pos, "神秘的地下湖，可能有特殊生物")
		features.append(feature)
...
func update_cave_ecosystem(creatures: Array[CreatureSpawn], delta: float) -> void:
	"""更新洞穴生态系统"""
	var rats = creatures.filter(func(c): return c.creature_type == CreatureTypes.CreatureType.GIANT_RAT)
...
func update_rat_colony_behavior(rats: Array[CreatureSpawn]) -> void:
	"""更新巨鼠群体行为"""
...
func update_cave_environment_adaptation(rats: Array[CreatureSpawn]) -> void:
	"""更新洞穴环境适应"""
	for rat in rats:
		if rat.is_active:
			# 巨鼠适应黑暗环境
