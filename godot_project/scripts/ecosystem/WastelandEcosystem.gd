...
func generate_wasteland_resources(region: EcosystemRegion) -> Array[ResourceSpawn]:
	"""生成荒地资源"""
	...
	for i in range(rare_mineral_count):
		...
		var resource = ResourceSpawn.new(ResourceTypes.ResourceType.RARE_MINERAL, Vector3(pos.x, 0, pos.y), amount, 2400.0)
	...
	for i in range(stone_count):
		...
		var resource = ResourceSpawn.new(ResourceTypes.ResourceType.STONE, Vector3(pos.x, 0, pos.y), amount, 900.0)
		resources.append(resource)
...
func generate_wasteland_creatures(region: EcosystemRegion) -> Array[CreatureSpawn]:
	"""生成荒地生物"""
	...
	for i in range(lizard_count):
		...
		var creature = CreatureSpawn.new(CreatureTypes.CreatureType.GIANT_LIZARD, Vector3(pos.x, 0, pos.y), level, true, 1800.0)
...
func generate_wasteland_features(region: EcosystemRegion) -> Array[RegionFeature]:
	"""生成荒地特殊特性"""
	...
	for i in range(abandoned_building_count):
		var pos = region.get_random_point()
		var feature = RegionFeature.new("abandoned_building", pos, "废弃的建筑，可能有稀有矿物")
	...
	for i in range(radiation_zone_count):
		var pos = region.get_random_point()
		var feature = RegionFeature.new("radiation_zone", pos, "危险的辐射区域，但可能有稀有资源")
		features.append(feature)
	...
	for i in range(oasis_count):
		var pos = region.get_random_point()
		var feature = RegionFeature.new("desert_oasis", pos, "沙漠中的绿洲，生命之源")
		features.append(feature)
	...
	for i in range(mineral_rich_zone_count):
		var pos = region.get_random_point()
		var feature = RegionFeature.new("mineral_rich_zone", pos, "矿物富集区域，稀有矿物丰富")
		features.append(feature)
...
func update_wasteland_ecosystem(creatures: Array[CreatureSpawn], delta: float) -> void:
	"""更新荒地生态系统"""
	var lizards = creatures.filter(func(c): return c.creature_type == CreatureTypes.CreatureType.GIANT_LIZARD)
...
func update_harsh_environment_pressure(lizards: Array[CreatureSpawn]) -> void:
	"""更新恶劣环境压力"""
	for lizard in lizards:
		if lizard.is_active:
			# 荒地环境对生物的影响
...
func update_lizard_territory_behavior(lizards: Array[CreatureSpawn]) -> void:
	"""更新巨蜥领地行为"""
	for lizard in lizards:
		# 巨蜥守卫领地
